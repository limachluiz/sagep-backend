...
function consolidated(report: CommitmentNoteReportData) {
  ...
  <section class="panel page-break"><div class="section-head"><div><small>CARTEIRA ANALÍTICA</small><h2>Notas de Empenho do recorte</h2></div><span>${report.rows.length} registro(s)</span></div>
    ...
    ${report.rows.map((row) => `<tr><td><strong>${esc(row.number)}</strong><small>${esc(originLabel(row.origin))}${row.project ? ` · PRJ-${row.project.projectCode}` : ""}</small></td><td>${esc(row.managementUnit)}</td><td>${esc(row.supplierName)}<small>${esc(row.supplierCnpj || "CNPJ não informado")}</small></td><td>${money(row.current)}</td><td>${liquidatedMoney(row.liquidated)}</td><td>${paidMoney(row.paid)}${(row.deductions ?? 0) > 0 ? `<small>OB ${money(row.paidNet ?? null)} + DR/DF ${money(row.deductions ?? null)}</small>` : ""}</td><td><span class="status">${esc(statusLabel(row.status))}</span>${row.incomplete ? "<small>A conferir</small>" : ""}</td><td>${date(row.updatedAt, true)}</td></tr>`).join("")}
    </tbody></table>
  </section>`;
}
