...
export function evaluateEnvironment(values) {
  ...
  checks.push(item("env.hostname", fqdnReady ? "PASS" : "FAIL", fqdnReady ? "O nome DNS completo foi informado." : "SAGEP_HOSTNAME não contém um FQDN válido.", "Informe o nome DNS completo definido para esta instalação."));
  ...
  checks.push(item("env.allowed-networks", networksReady ? "PASS" : "FAIL", networksReady ? `${new Set(allowedNetworks).size} rede(s) IPv4 privada(s) preparada(s) para o firewall.` : "SAGEP_ALLOWED_NETWORKS está vazia ou contém CIDR inválido, público ou não canônico.", "Informe os CIDRs privados da instalação separados por vírgula."));
