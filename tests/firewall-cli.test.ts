...
describe("firewall por CIDR", () => {
  it("normaliza somente CIDRs privados e bloqueia listas perigosas", () => {
    expect(parseAllowedNetworks("192.168.0.0/16, 172.16.0.0/16,172.16.0.0/16")).toEqual([
      "172.16.0.0/16",
      "192.168.0.0/16",
    ]);
    expect(() => parseAllowedNetworks("0.0.0.0/0")).toThrow(/inválido/);
    expect(() => parseAllowedNetworks("192.168.1.1/16")).toThrow(/canônico/);
  ...
  it("autoriza as redes declaradas e rejeita o restante", () => {
    expect(desiredChainRules("SAGEP-INGRESS-A", ["192.168.0.0/16"])).toEqual([
      ["-A", "SAGEP-INGRESS-A", "-s", "192.168.0.0/16", "-j", "RETURN"],
      ["-A", "SAGEP-INGRESS-A", "-p", "tcp", "-j", "REJECT", "--reject-with", "tcp-reset"],
    ]);
  });

  it("restringe somente 80/443 no IPv4 publicado pelo SAGEP", () => {
    const rules = desiredJumpRules("SAGEP-INGRESS-A", "192.168.50.20");
    expect(rules).toHaveLength(2);
    expect(rules.map((rule) => rule[rule.indexOf("--ctorigdstport") + 1])).toEqual(["80", "443"]);
    expect(rules.every((rule) => rule.includes("192.168.50.20") && rule.includes("ORIGINAL"))).toBe(true);
